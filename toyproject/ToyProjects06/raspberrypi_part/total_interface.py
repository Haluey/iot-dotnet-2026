...
# YOLO MQTT 중복전송 방지
YOLO_SEND_INTERVAL = 5.0    # 5초 동안 지연
...
def publish_arduino_data(client, serial_data: str):
    try:
        ...
        data = serial_data  # R, G, B 등의 일반 문자열
...
def serial_receive_thread(client):
    ...
    while running:
        try:
            ...
            print(f'Serial RX 오류 : {error}')
            time.sleep(1)

        except Exception as error:
            print(f'Serial Thread 오류 : {error}')
            time.sleep(1)
        ...
        for box in results[0].boxes:
            ...
            if (
                ...
                if (
                    ...
                    last_yolo_time = class_name
                    last_yolo_class = curr_time
