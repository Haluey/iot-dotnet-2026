...
class DetectionResult(BaseModel):
    message: str    # 객체인식 결과메시지
    image: str      # 인식결과 이미지

## 이미지 객체탐지 함수
def detectObjects(image: Image.Image):
    origin = np.array(image)    # PILLOW 이미지를 numpy 배열로 변환
    # 인식 후 리턴타입 list[ultralytics.engine.results.Results]
    ...
    for obj in objects: # 인식된 물체 개수만큼 반복
        ...
        for (box, conf, class_id) in zip(boxes, confidences, class_ids):
            ...
            cv2.putText(origin, f'{label} {conf:.2f}', (x1 + 7, y1 + 15),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2)

    result = Image.fromarray(origin)    # 결과를 다시 PILLOW 변환
    return result
...
# ASP.NET Core Website에서 form양식에서 넘어온 메시지와 이미지로 객체 인식
