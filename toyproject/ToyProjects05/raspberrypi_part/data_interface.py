...
## MQTT initialization
PUB_ID = 'IOT62-RPI' # 본인 아이피 마지막주소
BROKER = '210.119.12.62' # 본인 아이피
PORT = 1883
...
DATA_TOPIC = 'smartfactory/62/data'

# subscribe topic : WIN -> RPI -> Arduino
CONTROL_TOPIC = 'smartfactory/62/control'
...
## Data transfer to Arduino
def send_to_arduino(command: str):
    # from MQTT command to Arduino
...
def on_connect(client, userdata, flags, reason_code, properties=None):
    ...
    if reason_code == 0:
        ...
        client.subscribe(CONTROL_TOPIC, qos=1)  # qos가 1이면 UDP방식과 비슷(어떤 때든 한 번만 보내기 때문에)
